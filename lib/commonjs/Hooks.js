...
Object.defineProperty(exports, "useCallScreening", {
  enumerable: true,
  get: function () {
    return _CallScreening.useCallScreening;
  }
});
...
const SipContext = exports.SipContext = /*#__PURE__*/(0, _react.createContext)({
  agents: [],
  chatRooms: [],
  callLogs: []
});
const SipDispatchContext = exports.SipDispatchContext = /*#__PURE__*/(0, _react.createContext)(null);
 ...
 */
function usePushNotification(agentId) {
  const {
    agents
  } = (0, _react.useContext)(SipContext);
  const agent = agents.find(a => a.id === agentId);
  const [pushConfig, setPushConfig] = (0, _react.useState)(() => {
    if (!agent) return null;
    return (0, _PushNotification.parsePushContactParams)(agent.getConfiguration().contactParams ?? '');
  });
  const registerPushToken = (0, _react.useCallback)(async config => {
    if (!agent) {
      console.warn('usePushNotification: agent not found for id', agentId);
      return;
    }
    const contactParams = (0, _PushNotification.buildPushContactParams)(config);
    await agent.setContactParameters(contactParams);
    setPushConfig(config);
  }, [agent, agentId]);
  const clearPushToken = (0, _react.useCallback)(async () => {
    if (!agent) return;
